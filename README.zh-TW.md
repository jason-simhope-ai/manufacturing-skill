...
| 階段                       | 月成本                | 一次性                 |
| -------------------------- | --------------------- | ---------------------- |
| 試試看（雲端 Pro）         | $17-20 / 月（~NT$650） | 0                      |
| 重度使用（雲端 Max）       | $100-200 / 月         | 0                      |
| 圖紙稽核需求（地端 LLM）   | 電費                  | NT$200K 起（GB10 硬體） |

**對比參考**：找 SI 客製一套類似系統的傳統行情約 **NT$300K-1M 一次**（含開發、整合、培訓）— 本 plugin 是 fork-friendly open source，省的就是這塊。

> 💡 月繳不綁約，先用 Pro 試 1-2 個月（總成本約 NT$650-1,300）再決定要不要升級或自架。Anthropic 通常給新用戶一些免費 credit 試。
